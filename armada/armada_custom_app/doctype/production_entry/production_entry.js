        ...
        callback: function(r) {
            ...
            print_zpl_with_qz(r.message)
                .then(function() {
                    ...
                    frappe.show_alert({
                        message: __("Barcode labels sent to printer"),
                        indicator: "green"
                    });
                })
                .catch(function(error) {
                    frappe.msgprint({
                        title: __("Barcode Print Failed"),
                        indicator: "red",
                        message: get_qz_error_message(error)
                    });
                });
        }
    });
    ...
    return load_qz_tray()
        ...
        .then(function(printer_name) {
            ...
            return qz.print(config, [{
                type: "raw",
                format: "command",
                data: build_production_barcode_zpl(label)
            }]);
        });
}
...
function load_qz_tray() {
    ...
    return new Promise(function(resolve, reject) {
        ...
        script.onerror = function() {
            reject(__("Could not load QZ Tray JavaScript library"));
        };
...
function connect_qz_tray() {
    if (!window.qz) {
        return Promise.reject(__("QZ Tray JavaScript library is not available"));
    }
...
function get_qz_printer() {
    ...
    if (saved_printer) {
        return Promise.resolve(saved_printer);
    }

    return get_qz_printer_options().then(function(printers) {
        return new Promise(function(resolve, reject) {
            if (!printers.length) {
                reject(__("No printers found in QZ Tray"));
                return;
            }
            ...
            dialog.$wrapper.on("hidden.bs.modal", function() {
                if (!selected) {
                    reject(__("Barcode printer was not selected"));
                }
            });
...
function get_qz_printer_options() {
    if (qz.printers.details) {
        return qz.printers.details().then(function(details) {
            return details.map(function(printer) {
                return printer.name || printer;
            }).filter(Boolean);
        });
    }

    return qz.printers.find().then(function(printer) {
        return printer ? [printer] : [];
    });
}

function build_production_barcode_zpl(label) {
    const PW = 812; // 10cm at 203 DPI
    const LL = 1218; // 15cm at 203 DPI
    const Y_START = 70;
    ...
    const estimated_barcode_width =
        (barcode_value.length * 11 + 35) * barcode_module;

    let barcode_y =
        Y_START +
        Math.floor((FULL_WIDTH - estimated_barcode_width) / 2);
    ...
    const item_code_graphic = render_text_graphic(item_code_text, {
        width: FULL_WIDTH,
        height: 150,
        font_size: item_font.height,
        max_width: FULL_WIDTH - 180,
        min_font_size: 56,
        weight: 800
    });
    const barcode_text_graphic = render_text_graphic(barcode_value, {
        width: FULL_WIDTH,
        height: 80,
        font_size: 66,
        weight: 800
    });
    const date_graphic = render_text_graphic("Date: " + datetime, {
        width: FULL_WIDTH,
        height: 68,
        font_size: 54,
        weight: 800
    });
    const fp_type_graphic = render_text_graphic(fp_type_text, {
        width: COL_WIDTH,
        height: 68,
        font_size: fp_segment_font.height,
        weight: 800
    });
    const segment_graphic = render_text_graphic(segment_text, {
        width: COL_WIDTH,
        height: 68,
        font_size: fp_segment_font.height,
        weight: 800
    });
    const product_type_graphic = render_text_graphic(product_type_text, {
        width: COL_WIDTH,
        height: 64,
        font_size: product_standard_font.height,
        weight: 800
    });
    const standard_graphic = render_text_graphic(standard_text, {
        width: COL_WIDTH,
        height: 64,
        font_size: product_standard_font.height,
        weight: 800
...
function shift_label_y(value, shift) {
...
function canvas_to_zpl_gfa(canvas) {
    ...
    for (let y = 0; y < canvas.height; y++) {
        for (let byte_index = 0; byte_index < bytes_per_row; byte_index++) {
            ...
            for (let bit = 0; bit < 8; bit++) {
                ...
                const red = image_data[offset];
                const green = image_data[offset + 1];
                const blue = image_data[offset + 2];
                const alpha = image_data[offset + 3];
    ...
    return {
        data: "^GFA," + total_bytes + "," + total_bytes + "," + bytes_per_row + "," + hex,
        width: canvas.width,
        height: canvas.height
    };
}

function zpl_graphic(x, y, graphic) {
    return "^FO" + x + "," + y + graphic.data + "^FS";
}
...
function zpl_text(value) {
    return String(value || "").replace(/[\^~\\]/g, " ").slice(0, 80);
}

function zpl_barcode(value) {
