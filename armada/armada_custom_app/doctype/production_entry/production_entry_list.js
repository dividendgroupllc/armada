frappe.listview_settings["Production Entry"] = {
	onload(listview) {
...
function render_qty_summary(listview) {
	...
	// Count only actually-manufactured (Submitted) documents
	filters.push(["Production Entry", "docstatus", "=", 1]);
	...
	frappe.db
		.get_list("Production Entry", {
			filters: filters,
			fields: ["sum(qty_to_manufacture) as total"], // single aggregate => no ORDER BY
			limit: 0,
		})
		.then((rows) => {
			if (listview.__qty_summary_token !== token) return; // a newer request started
			const total = (rows && rows.length && rows[0].total) || 0;
			// Show as integer (no decimals) — you can't manufacture 2.3 items
...
function ensure_summary_label(listview) {
	...
	$label = $(`
		<div class="armada-qty-summary text-muted"
			...
			<span>${__("Ishlab chiqarilgan")}:</span>
