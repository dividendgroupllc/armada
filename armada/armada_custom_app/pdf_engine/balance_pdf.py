...
Follows exact same pattern as pl_pdf.py
"""
import os
...
# ─── LAYOUT CONSTANTS (extracted from balance template PDF) ──────────────────
TEMPLATE_COL_X = [296.1, 371.0, 445.9, 520.8, 595.6, 670.5, 745.4, 820.3]
LABEL_CUTOFF   = 250.0      # label zone < 250, data zone >= 250

# Header rows — skip template text, draw dynamically
HEADER_TOPS = {46, 59, 70, 81}   # #REF!, col numbers, Год, Месяц

# Rows to COMPLETELY skip (artifacts from template)
SKIP_TOPS = {46}   # #REF!53678.62875
...
# ─── ROW MAP: template top → (data_key, format) ─────────────────────────────
# Extracted from pdfplumber: round(ch["top"]) for each data row
ROW_MAP = {
    # ── Активы ──
    # Внеоборотные активы
    ...
    260: ("empty_cash_row",         "num"),     # unnamed zero-row
...
# ── Y cutoff: skip everything below Разница row (analysis section removed) ──
CUTOFF_TOP = 755
...
def _fmt(v, style):
    ...
    if style == "plain":
        return f"{v:.2f}"
    # "num" — default comma-separated, negative in parens
...
def _derive(data, n):
    """
    Calculate totals and ratios from raw account balances.
    All inputs are lists of length n (one per month/column).
    """
    def g(k):
    ...
    debit_zadolzh = [zadolzh_klientov[i] + avansy_postav[i]
...
def _draw_dynamic_header(cv, col_keys, col_x, page):
    """
    Draw Год and Месяц rows dynamically from col_keys.
    Template text in header zone is skipped; we re-draw with actual data.
    """
    # ── Год row (top≈70) ──
    ...
    # ── Column number row (top≈59): draw sequential numbers ──
...
def generate(data: dict,
    ...
    data:  dict of { key: [val_month1, val_month2, ...], ... }
           Keys matching ROW_MAP values. Extra keys _revenue, _cogs,
           _monthly_revenue used for ratio calculations.
    col_keys: list of column identifiers like ["may_2025", "jun_2025", ...]
    """
    register_fonts()
    ...
    # Merge raw data with calculated fields
    ...
    with pdfplumber.open(template) as pdf:
        ...
        # 1. ── Backgrounds (skip below CUTOFF_TOP) ──
        ...
        # 3. ── Draw rows (skip below CUTOFF_TOP) ──
        for top_key in sorted(rows.keys()):
            if top_key >= CUTOFF_TOP:
                continue
            chars = rows[top_key]

            # SKIP artifacts (#REF!, stray numbers)
            if top_key in SKIP_TOPS:
                continue

            # SKIP header data zone — draw labels only, data is dynamic
            if top_key in HEADER_TOPS:
                for ch in chars:
                    ...
                    cv.setFont(resolve_font(ch.get("fontname", "")),
                               ch.get("size", 6.90))
                    cv.setFillColor(to_color(
                        ch.get("non_stroking_color", (0, 0, 0))))
                    cv.drawString(ch["x0"], rl_y(ch["bottom"]), txt)
                continue

            # ── Label zone: always draw ──
            for ch in chars:
                txt = ch.get("text", "")
                if not txt.strip() or ch["x0"] >= LABEL_CUTOFF:
                    continue
            ...
            # ── Data zone: ROW_MAP match → inject dynamic values ──
            ...
            if best_dist <= 6 and best_key is not None:
                ...
                # Special styling for calculated rows
                ...
                # Baseline from data chars (right side) or label chars
